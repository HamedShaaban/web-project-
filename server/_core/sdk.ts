...
class SDKServer {
  ...
  private getSessionSecret() {
    const secret = ENV.cookieSecret || "dev-jwt-secret-key-at-least-32-characters-long";
  ...
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (!user) {
      if (!ENV.oAuthServerUrl || session.openId.startsWith("dev-") || session.openId.startsWith("guest-")) {
        user = {
          id: 1,
